...
def parse_time_control(time_control: str):
    ...
    """
    if not time_control:
        raise ValueError("PGN has no TimeControl header.")
    if "+" not in time_control:
        raise ValueError(f"Unsupported TimeControl: {time_control!r}. " "Expected something like '30+2'.")
    ...
    try:
        ...
        raise ValueError(f"Could not parse TimeControl: {time_control!r}")
...
def get_search_result(engine, moves, wtime, btime, winc, binc):
    """
    ...
    engine.stdin.write(f"go eval " f"wtime {wtime} " f"btime {btime} " f"winc {winc} " f"binc {binc}\n")
    engine.stdin.flush()
    ...
    while True:
        ...
        if line.startswith("info string"):
            print(f"    {line}")  # surface diagnostics
    ...
    while True:
        line = engine.stdout.readline()
        if not line:
            raise RuntimeError("Engine terminated during dumpstats.")
        line = line.strip()

        # The stats output ends with a line of '=' characters.
        if (in_search_stats and line.startswith("=")):
            break
        ...
        # Depth                           11
        # Completed Depth                 11
        match = re.match(r"Depth\s+(\d+)", line)
        if match:
...
def main():
    parser = argparse.ArgumentParser(description=("Replay a PGN and search only Tomahawk's moves."))
    parser.add_argument("--pgn", required=True, type=Path, help="PGN file")
    parser.add_argument("--engine", required=True, type=Path, help="UCI engine executable")
    parser.add_argument("--all", action="store_true", help="Print every Tomahawk position.")
    parser.add_argument("--fen", action="store_true", help="Print FEN for differing positions.")
    parser.add_argument(
        "--problem_move",
        type=int,
        help="Move number where a move was played that needs to be investigated. Prints root moves with information."
    )
    args = parser.parse_args()

    # READ PGN
    with args.pgn.open("r", encoding="utf-8") as f:
        game = chess.pgn.read_game(f)
    if game is None:
        print("ERROR: Could not read PGN.", file=sys.stderr)
        sys.exit(1)
    ...
    try:
        ...
        sys.exit(1)
    engine_side_name = ("White" if engine_color == chess.WHITE else "Black")
    ...
    # SUMMARY
    print(f"Game: " f"{game.headers.get('White', '?')} - " f"{game.headers.get('Black', '?')}")
    print(f"Tomahawk side: {engine_side_name}")
    print(
        f"TimeControl: {time_control} "
        f"(initial={format_clock(initial_time)}, " f"increment={increment / 1000:g}s)"
    )
    ...
    try:
        ...
        for node in game.mainline():
            ...
            if (current_color == engine_color) and (opening_counter > 8):
                ...
                result = get_search_result(engine, moves, search_wtime, search_btime, increment, increment)
                engine_move = result["bestmove"]

                different = (engine_move is not None and engine_move != game_move)
                ...
                if args.all or different:
                    eval_string = "?" if result["eval_cp"] is None else f"{result['eval_cp'] / 100:+.2f}"
                    engine_clock = (search_wtime if engine_color == chess.WHITE else search_btime)
                    print(
                        f"{label:<8}"
                        f"{game_move:<8}"
                    ...
                    # remove dot from label '1.'
                    """
                    if int(label[:-1]) == args.problem_move:
                        engine.stdin.write("dumpmoves\n")
                        engine.stdin.flush()

                        while True:
                            line = engine.stdout.readline()
                            if not line:
                                print("ENGINE EOF", flush=True)
                                break

                            print(f"ENGINE: {line}", end="", flush=True)
                            if line.strip() == "done.":
                                print("GOT DONE", flush=True)
                                break
                    """

                    if different:
                        print(f"    DIFFERENCE: " f"game={game_move}, " f"engine={engine_move}")
                        if args.fen:
            ...
            post_move_clock = parse_clock(node.comment)
            if post_move_clock is None:
                print(f"WARNING: No clock found for " f"{label} {game_move}", file=sys.stderr)
        ...
        print()
        print("=" * 70)
        print(f"Tomahawk positions searched: {positions_searched}")
        print(f"Move-choice differences:      {differences}")
        print("=" * 70)
